...
import com.example.administrator.roczhengplayer.util.LyricLoader
import com.example.administrator.roczhengplayer.util.LyricUtil
...
class LyricView : View {
    ...
    var updateByPro = true//指定是否可以通过progress进度来更新歌词
    ...
    private fun drawManyLine(canvas: Canvas?) {
        if (updateByPro) {//通过进度更新歌词
            ...
            var offsetTiem = progress - list.get(centerLine).startTime
            //偏移百分比=偏移时间/行可用的时间
            var offsetPercent = offsetTiem / (lineTime.toFloat())
            //偏移y=偏移百分比*行高
    ...
    fun updateProgress(progress: Int) {
        ...
        if (progress >= list.get(list.size - 1).startTime) {
            ...
            for (index in 0 until list.size - 1) {
                //progress>=当前行的开始时间&progress<下一行开始时间
    ...
    fun setSongName(name: String) {
        doAsync {
            ...
            this@LyricView.list.addAll(LyricUtil.parseLyrice(LyricLoader.loadLyricFile(name)))
        }
    }

    /**
     * 歌词控件手势事件处理
     * 1,手指按下时，停止通过进度更新歌词操作
     */
    var downY = 0f
    var markY = 0f//原来已经偏移的Y值
